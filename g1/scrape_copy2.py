...
def get_links(driver1):
    ...
    all_links_sliced = []
    for w in all_links:
        all_links_sliced[w] = all_links[]
    send_links = []
    count = 0
    f = open('already_deal', 'r')
    x = f.read().split(',')
    print(x)
    for i in all_links:
        if all_links[count] not in x:
            send_links.append(all_links[count])
            x.append(all_links[count[0:37]])
        count += 1
    f.close()
    x = ','.join(x)
    f = open('already_deal', 'w')
    f.write(x)
    f.close()
    print(all_links)
    print(send_links)
